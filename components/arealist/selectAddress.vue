		...
		<transition name="content">
			<view class="content_view" v-show="isShow">
				...
				<view class="select_top">
					...
					<view class="btn" @click="hidden">确定</view>
						...
						<view class="swiper-item">
							<scroll-view class="scroll-view-item" scroll-y="true">
								<view class="address_item" v-for="(item,index) in cityAreaArray[swiper_index]" :key="index" @click="address_item_click(swiper_index,index)">
									<view :class="selectIndexArr[swiper_index] === index?'address_item_text':''">{{item.name}}</view>
								</view>
							</scroll-view>
						</view>
					</swiper-item>
...
<script>
	...
	export default {
		...
		methods: {
			...
			hidden() {
				if(this.dataList[0] == "省份"){
					this.util.Toast("请选择省份")
					return
				}
				if(this.dataList[1] == "市级"){
					this.util.Toast("请选择市级")
					return
				}
				if(this.dataList[2] == "区县"){
					this.util.Toast("请选择区县")
					return
				}
				this.isShow = false
				setTimeout(() => {
			...
			changeIndicator(index) {
				...
				*/
				...
				arr.fields({
					...
					let itemCenterX = 16 + index * 80 + itemWidth / 2
			...
			address_item_click(swiper_index, index) {
				...
				if (swiper_index === 0) { //第一级
					...
					this.dataList.splice(swiper_index + 1, 0, '市级')
					...
					if (area !== undefined) {
						...
						this.dataList.splice(swiper_index, 5, city)
						this.dataList.splice(swiper_index + 1, 0, '区县')
